# Ferlab-Ste-Justine/seq-data-validation: Usage
...
```bash
nextflow run Ferlab-Ste-Justine/seq-data-validation --input ./samplesheet.csv --outdir ./results  -profile docker
```
...
```bash
nextflow run Ferlab-Ste-Justine/seq-data-validation -profile docker -params-file params.yaml
```
...
```bash
nextflow pull Ferlab-Ste-Justine/seq-data-validation
```
...
First, go to the [Ferlab-Ste-Justine/seq-data-validation releases page](https://github.com/Ferlab-Ste-Justine/seq-data-validation/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
